# The following is a modified version of ursina's entity.py, for the purpose of increasing performance in specific functions, relevant only to this game.
...
class Entity(NodePath, metaclass=PostInitCaller):
    ...
    def __post_init__(self):
        # Only entities that actually have behaviour (update/input) need
        # to live in scene.entities and be touched every frame.
        is_dynamic = hasattr(self, 'update') or hasattr(self, 'input')

        if self.add_to_scene_entities and is_dynamic:
            scene.entities.append(self)

        # Keep on_enable/on_disable behaviour as before
    ...
    def _ensure_is_not_destroyed(self):
        # Turn this into a no-op for performance.
        return
